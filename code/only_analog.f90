...
! =================
real function obs_bias(cur_fc,past_fc,past_obs,n_members)
implicit none
real*8 :: cur_fc,m,q,ymean,xmean,deltaf
real*8 :: past_fc(n_members),past_obs(n_members)
integer n_members

ymean=sum(past_obs(:))/n_members
xmean=sum(past_fc(:))/n_members

if(sum((past_fc(:)-xmean)**2).ne.0.)then
 m=sum((past_fc(:)-xmean)*(past_obs(:)-ymean))/(sum((past_fc(:)-xmean)**2))
!slope for least square equation, a linear relationschip between fc and obs
!isassumed
else
 ! print*,"Attention,all past analog forecast and past corresponding obs are =
 ! 0. Bias corr.  set to  0."
  m=0.
endif
...
subroutine only_analog (obs, pred, vmiss, apar, fpar, kpar, ratio, diag,           &
    ...
    lower_limit_target,upper_limit_target,method,bias_cor)
  ...
  ! ==== WC: 2015-06-22; 2016-08-05  ===
  integer,         intent(in ) :: method                ! option for ANKF
 
  integer,         intent(in ) :: bias_cor              ! option for bias correction
...
!!  character filename*40		! debug only
  ...
  integer winLower, winUpper, nanalogs,iday,nmaxday
  real forecast_step
...
!      print*,"d-1,winl winup",vmiss
       ...
       nmaxday=nint(real(nhour)*forecast_step/24.)
...
!       forecast_step=1
      if(nmaxday>0)then
       do iday=1,nmaxday
        ...
        p_time=nint(24./forecast_step*real(iday) - real(hour_start)/forecast_step + 2 )
...
!!						     ! for AN (until 9 Jan 12)
        ...
        if(bias_cor==0)then
         ...
         bias=obs_bias(pred(d,h,fvar),predtemp(Ian),obstemp(Ian),ct)
         analog_in_an(d, h, a1:a2) = obstemp(Ian)+bias
        endif
