...
!
! 2016-jan-14	Dave A:  Cosmetic changes only, for readability.
!		Add block statement labels, etc.
!
!-----------------------------------------------------------------------------
...
!  character(200) text_dir, out_template
...
!  integer di, hi, prova
  ...
  integer :: method, atec_file_format, bias_cor
...
namelist / anen_parameters / nday_start, num_lead_times, num_analogs, hour_start, forecast_freq, &
           ...
           opt_metric_endday, window, trend, method, bias_cor, atec_file_format
...
!ste now it is provided as an option from analog_input.nml
...
!ste  obs_file   = 'data/combined_data.20140425.12z.nc'
  ...
  bias_cor           = 0        ! bias correction is turned off by default
...
!ste flipping the order to allow setting in analog_input.nml
! the order provided by ncdump
...
!ste                                                                        ! LDM
 ...
 if(.not.find_weight_file) allocate (analog_in_an_w(ndays, num_lead_times, nstations, apar%num_an))  ! DHA
...
!ste  obs = obs_data(:, :, iobs_var, si)	! (days, hours, vars, stations)
      obs = obs_data(:, 1:num_lead_times, iobs_var, si)
       			! (days, hours, vars, stations) --> (days, hours)

!ste  ens = ens_data(:, :, :,        si)	! (days, hours, vars, stations)
      ens = ens_data(:, 1:num_lead_times, :,        si)
        		! (days, hours, vars, stations) --> (days, hours, vars)
...
icomb_loop:&
       do icomb=1,cont
         ...
         call only_analog (obs, ens, vmiss, apar, fpar, kpar, ratio, diag, &
                  ...
                  lower_limit_target,upper_limit_target,method,bias_cor)  ! LDM !ste
...
!ste case weight_ini.txt already exists
      ...
      call only_analog (obs, ens, vmiss, apar, fpar, kpar, ratio, diag,     &
                   ...
                   hour_start,lower_limit_target,upper_limit_target,method,bias_cor)  ! LDM !ste
    end if find_wf
...
!! call exit
 ...
 print *,"Collecting vectors from nodes"
...
! Assign units
        call check ( nf90_put_att(ncid, days_varid, "units", &
                                          "number of forecast days") )
        call check ( nf90_put_att(ncid, times_varid, "units", &
                                          "number of forecast lead times") )
        call check ( nf90_put_att(ncid, stats_varid, "units", &
                                          "number of station locations") )
        call check ( nf90_put_att(ncid, nummem_varid, "units", &
                                          "number of analog ensemble members") )
...
print *,"Successfully writing netCDF file", out_file_netcdf
    ...
    end if main_proc_output		! end of if(myrank==0)
